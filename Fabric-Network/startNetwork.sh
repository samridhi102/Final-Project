...
export ORDERER_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/ticket.com/orderers/orderer.ticket.com/tls/server.crt

export ORDERER_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/ticket.com/orderers/orderer.ticket.com/tls/server.key
...
echo "—---------------Join organiser peer to the channel—-------------"
...
echo "—-------------organiser anchor peer update—-----------"

peer channel fetch config ${PWD}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer.ticket.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA
sleep 1
...
peer channel update -f ${PWD}/channel-artifacts/config_update_in_envelope.pb -c $CHANNEL_NAME -o localhost:7050  --ordererTLSHostnameOverride orderer.ticket.com --tls --cafile $ORDERER_CA
sleep 1

echo "—---------------package chaincode—-------------"

peer lifecycle chaincode package finalproject.tar.gz --path ${PWD}/../Chaincode/ --lang golang --label finalproject_1.0
sleep 1

echo "—---------------install chaincode in organiser peer—-------------"
...
echo "—---------------Approve chaincode in organiser peer—-------------"
...
echo "—---------------Join reseller peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------reseller anchor peer update—-----------"
...
echo "—---------------install chaincode in reseller peer—-------------"
...
echo "—---------------Approve chaincode in reseller peer—-------------"
...
echo "—---------------Join attendee peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------attendee anchor peer update—-----------"
...
echo "—---------------install chaincode in attendee peer—-------------"
...
echo "—---------------Approve chaincode in attendee peer—-------------"

peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.ticket.com --channelID $CHANNEL_NAME --name Final-Project --version 1.0 --collections-config ../Chaincode/collection.json --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 1

echo "—---------------Commit chaincode in attendee peer—-------------"
